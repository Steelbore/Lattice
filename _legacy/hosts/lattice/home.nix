...
in
{
  ...
  programs = {
    ...
    git = {
      ...
      #};
      signing = {
        # Replace this with the 40-character fingerprint from Step 1
      ...
      settings = {
        ...
        gpg = {
          # Explicitly tell Git to use the OpenPGP format
          format = "openpgp";
          # Hardcode the path to the Sequoia shim in the Nix store
          program = "${pkgs.sequoia-chameleon-gnupg}/bin/gpg";
          # Ensure the shim is installed in your user environment
        };
      };
    };
